...
| 功能 | 说明 |
|------|------|
| 🌓 **即时主题切换** | 亮色/暖暗色纯 CSS 切换，**零延迟不刷新**，自动记忆偏好 |
| 📈 **实时数据看板** | K线周期选择器 + 腾讯行情 + 北向资金 + 概念板块 + 行业对比 |
| 📂 **可展开侧边栏** | 顶部 ☰ 按钮 + Streamlit 原生切换，股票输入/模型配置/历史记录 |
...
**切换机制**：纯 JavaScript/CSS 即时切换，localStorage 持久化，切换无需等待页面重载
...
- **🌓 即时主题切换**：顶部 ☀️亮色 / 🌙暗色 一键切换，**纯 JS/CSS 零延迟**，localStorage 自动记忆
- **📂 可展开侧边栏**：顶部 ☰ 按钮控制侧边栏展开/收起，内含股票输入、LLM 配置、历史记录
...
- **历史记录**：自动保存并展示所有历史分析，支持代码/日期搜索过滤
