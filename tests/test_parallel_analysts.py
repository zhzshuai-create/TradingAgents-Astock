...
def _edges_checks(edges):
    ...
    # counting-gate join: every analyst's terminal branch lands on the shared
    # "Analyst Join" node, which routes to QG only after all analysts reported
    join_gate = (
        "Analyst Join" in nodes
        and all(
            (f"{a} Analyst", "Analyst Join") in edges for a in _ANALYST_NODES
        )
        and ("Analyst Join", "Quality Gate") in edges
    )
    chained = any(
    ...
    return start_targets, has_clear_nodes, join_gate, chained, tool_loops


@pytest.mark.unit
def test_serial_topology_is_chain():
    edges = _build(False)
    start_targets, has_clear_nodes, join_gate, chained, tool_loops = _edges_checks(edges)
    assert start_targets == {"Market Analyst"}
    assert chained
    assert has_clear_nodes
    assert not join_gate
    assert tool_loops
...
def test_parallel_topology_fans_out():
    ...
    assert join_gate  # Quality Gate runs once, after ALL analysts reported
    assert not chained
    assert tool_loops
